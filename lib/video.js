    ...
    function request(client , meta) {
        // Send stored webm header to client
        if (ebmlHeader !== null) client.send(ebmlHeader);
    ...
    function upload(client, stream, meta) {
        ...
        stream.on('data', function (data) {
            ...
            // Manually parse the ebml header
            if (ebmlHeader === null) {
    ...
    function writeStreamToFile(){
        ...
        // TODO: Korjaa että toimii!
        
        if (vInd !== 1) fStream.write(new Buffer(ebmlHeader));
    ...
    function readebmlHeader(data) {
        ...
        for (var i = 0; i < buff.length; i++) {
            ...
            if (charEndBuff == "" && char == '\r') charEndBuff += char;
            else if (charEndBuff == "\r" && char == '\n') charEndBuff += char;
            else if (charEndBuff == "\r\n" && char == '\r') charEndBuff += char;
            else if (charEndBuff == "\r\n\r" && char == '\n') charEndBuff += char;
